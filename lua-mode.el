...
(defvar lua-emacs-menu
  '(["Restart With Whole File" lua-restart-with-whole-file t]
    ...
    ["Load File" lua-load-current-file t]
...
(defun lua-load-current-file ()
  (interactive)
  (let ((command (concat "loadfile([[" (buffer-file-name) "]])()")))
...
(defun lua-send-region (start end)
  ...
  (let* ((lineno (line-number-at-pos start))
         ...
         (command
          ...
          ;;        lineno)))
          ;;
          ;; It seems as if the only reason that the devs use lua-make-lua-string ... etc.
          ;; is to avoid multiple ">>" in the repl.
          ;; This problem could be mitigated by:
          ;;   (1) providing a load-file command.
          ;;   (2) not caring.  my thinking is that send-region is to be used on short
          ;;       pieces of text anyways.
          ;; So here we just send the region-str in its naked form.
          region-str))
    (lua-send-string command)
...
(define-key lua-mode-menu [send-file]
  '("Load File" . lua-load-current-file))
